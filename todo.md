...
- [ ] Find real users. Primary track is **Real World Adoption**, measured on verified users (pre-28-Aug Celo activity), returning users (2+ distinct days), and distinct signers/authorisers — not volume. EIP-3009 authorisers and sponsored-relay signers both count. Encode has **zero** independent signers, so every track that counts anything still reads zero.
...
- [ ] Deploy to a live URL and fill `demoUrl` / `appDomain`, both currently empty — the most visible gap a judge hits. `server/railway.json` already exists, so this is a deploy, not a build. Note the deploy needs a `GITHUB_TOKEN` for the incident pipeline, and it must run somewhere with **no ambient git credential helper** (see AGENTS.md).
- [ ] Push `3bba7ad` and the doc corrections. They are committed locally and **not on GitHub** — this machine has no GitHub credential at all (no helper, no `gh`, no token in env or on disk).

## Done

- [x] **Submission published 2026-09-17T23:04Z** (`POST /submissions/me/publish`, returned `status: published`). Carries `real-world-adoption` plus a Value Moved declaration, `celoNetwork: celo-mainnet`, and the real X post as `socialLink`. Description and tagline were both empty/absent on the draft and are now filled — including a plainly-stated "what does not work yet" section, because a submission for a track called Real World Adoption that has no users should say so itself rather than let a judge discover it.
- [x] **Deadline corrected everywhere.** Four files (`README.md`, `PROJECT.md`, `todo.md` ×2) said submissions closed **Sept 14, 09:00 UTC**. The real value is **`2026-09-21T09:00:00Z`**, read from `GET /hackathons/agents-at-work`. Nothing was lost — but the repo was confidently wrong about its own deadline for three weeks, in four places, while the live API disagreed. Same failure mode as the `/verify`-is-not-payment bug: an assumption that was never checked against the source.
- [x] **On-chain registration file's stale prices fixed** (`agent-registration.json`). It was still advertising triage $1.50 / fix $8.00 — a 16x overstatement on the fix tier — after `7ee34ec` dropped the real prices to $0.20/$0.50. Every other surface had been updated; this file was missed because nothing imports it, and it is what the ERC-8004 `agentURI` resolves to. No on-chain transaction needed.
...
- [x] **Registered at celobuilders.xyz** (2026-08-29). Attribution tag **`celo_4bec1b4754cb`**, participant `bd57f400…`, submission `3f022c8c…` (now **published**). Primary track `real-world-adoption`. The connection credential is a `sk-celo-hackathon_…` bearer token. It was lost once — `todo.md` claimed it was "held outside the repo" but it was held nowhere — so it now lives at `~/.config/celobuilders/token`, mode 600, and re-auth is the documented fallback (`POST /auth/google/start`, then `/claim`; reconnecting with the same Google account returns the same `participantId`).
...
- [x] **Rebuilt to the supplied reference design** as a three-column workspace: bone canvas (`#EAE8E1`), off-white nested panels, one electric-orange accent, Instrument Serif display over Inter/JetBrains Mono. Depth is surface separation and 1px hairlines — no drop shadows anywhere, which is the whole look. The columns carry real content rather than app chrome: left is Encode's verifiable history (every item links to the tx, the agent, or the PR), centre is the pitch → pricing → ledger → docs, right is live `/v1/status` and `/v1/dashboard`.
- [x] Dropped the "Standing watch" tier rather than advertising something unbuilt. Pricing is now the two tiers that exist, presented through the reference's segmented switch.
- [x] The ledger flags rows that were paid but not delivered ("paid · not delivered"), and the right column surfaces `unfulfilledPaid` with a note that no automatic refund path exists. A settled row that delivered nothing must not look like one that did.
- [x] Accessibility re-checked after the reskin: the reference's `#94928A` de-emphasised grey fails AA at 2.6:1, so `--faint` is `#67655E` and every sampled text/surface pair now passes (lowest 4.84:1). Verified with a real browser — skip link is the first tab stop and visible on focus, both tablists respond to arrow keys, focus rings survive the light palette, decorative SVGs are `aria-hidden`, the caret stops blinking under `prefers-reduced-motion`, and there's no horizontal overflow at 390px (where the centre column is ordered first).
...
- [x] Registered through the `celobuilders` skill — tag issued
- [x] Repo public and resolving: [`greyw0rks/encode`](https://github.com/greyw0rks/encode) returns 200 (rule: two entries got disqualified last hackathon for a 404'ing repo). **Must still resolve at judging** — re-check before Sept 21.
- [x] **Published** 2026-09-17T23:04Z, well inside the real deadline of **Sept 21, 09:00 UTC**
- [x] X/Twitter post published tagging @CeloDevs + @Celo with the ERC-8004 registry link, and its URL sent as `socialLink`: <https://x.com/Greyw0rks/status/2097598274450436418>
- [ ] `greyw0rks/encore-testbed` and its PR #1 are the demo artifact — a real Encode-authored fix on a real repo. The submission description cites PR #1 by name; keep the testbed repo and PR public so the cite resolves. Note the repo keeps the pre-rename name.
- [ ] The `PUT /submissions/me` endpoint is **not** a partial update: it requires `projectName` and `githubUrl` on every call, and returns `400` without them. Its errors also carry a `skillHint` that says your skill may be stale — that hint is generic and appears even when your copy is current (verified by sha256 against the live `SKILL.md`). Don't chase it.
