...
- Fully in-request (synchronous) refund. When a fix run can't be verified — the repo has no runnable `npm test` script **and** no runnable repro command — Encode now recognises that as a distinct outcome (`no_verification_signal`, not `verification_failed`), records it, and flags the incident `refundOwed`. It is **not** billed as a fix that was attempted and found wanting, and a repo with no test suite but a *confirmed* repro (fails before the patch, passes after) now ships a verified fix on the repro alone. Paying these debts back is a sweep, not an in-request code path: `npm run refund:auto` simulates refunding every `no_verification_signal` debt, `npm run refund:auto -- --broadcast` sends them (real USDC from the payout wallet, on the host that holds the DB volume). A `verification_failed` debt — where Encode *did* run a check and the fix was shown not to hold — is deliberately excluded from the auto sweep and left to a human. `npm run refund:list` shows everything owed; `npm run refund -- --incident <id>` refunds one.
