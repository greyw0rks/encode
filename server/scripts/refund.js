...
import { getIncident, listUnfulfilledPaid, refundIncident } from '../src/store/incidentStore.js';
...
async function main() {
  ...
  const token = new ethers.Contract(usdc, USDC_ABI, wallet);
  ...
  // Record it only after the chain confirms — the txHash is the proof, and
  // refundIncident refuses a double-refund keyed on it. When the record was
  // resolved from the live ledger (no local store), there's nothing to write
  // here: the money moved, but the production DB still shows the debt until
  // it's reconciled on the host that holds it.
  if (!canRecord) {
    console.log(
  ...
  const recorded = refundIncident(id, {
    txHash: tx.hash,
    amount: usd,
    asset: 'USDC',
    network: 'celo',
