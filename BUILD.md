...
- `GITHUB_TOKEN` as used needs write access to whatever repos Encode patches — fine for your own. For a repo it *can't* write to, Encode forks it into the token's own account and opens a cross-repo PR instead (`src/repo/delivery.js`). That needs nothing from the repo owner, but only works on **public** repos. A **private** repo Encode can't write to is undeliverable: `fix` refuses up front with `delivery_unavailable` rather than burning a patch and a verification run before failing at the push. Closing that needs the GitHub App / installation-token flow.
- **Forking is always on, and forks are never cleaned up.** Every `fix` on a public repo the token can't write to leaves a permanent fork under the token's account. This is deliberate — no config to get wrong — but the operator's GitHub account accumulates them, so decide before pointing Encode at a lot of client repos.
